...
SELECT 
    ...
    precio AS 'Euros', 
    ROUND(precio * 1.083, 2) AS 'Dólares'
FROM producto;
...
SELECT
    LOWER(nombre) AS 'Nombre minúsculas',
    precio
FROM producto;
...
SELECT 
    nombre,
    UPPER(LEFT(nombre, 2)) AS 'Dos primeras letras en mayúsculas'
FROM fabricante;
...
SELECT p.nombre AS "Nombre de productos marca Lenovo"
FROM producto p
...
SELECT p.nombre AS "Productos marca Crucial"
FROM producto p
JOIN fabricante f ON p.codigo_fabricante = f.codigo
